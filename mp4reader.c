	...
	if (mp4->mediafp)
	{
		...
		do
		{
			...
			if (len == 8 && mp4->filepos < mp4->filesize)
			{
				...
				for (int i = 1; i < nest; i++) printf(" %5d ", (int)nestsize[i]); //printf("    ");
				printf(" %c%c%c%c (%lld)\n", (qttag & 0xff), ((qttag >> 8) & 0xff), ((qttag >> 16) & 0xff), ((qttag >> 24) & 0xff), (long long int)qtsize);
...
#if PRINT_MP4_STRUCTURE
							printf(" type/subtype: %c%c%c%c/", (type & 0xff), ((type >> 8) & 0xff), ((type >> 16) & 0xff), ((type >> 24) & 0xff));
							printf("%c%c%c%c\n", (subtype & 0xff), ((subtype >> 8) & 0xff), ((subtype >> 16) & 0xff), ((subtype >> 24) & 0xff));
#endif
...
{	
	size_t mp4handle = 0;
	if(argc >= 2)
		mp4handle = OpenMP4Source(argv[1], MOV_GPMF_TRAK_TYPE, MOV_GPMF_TRAK_SUBTYPE, 0);
	else
		printf("usage: mp4struct <path_to_an_mp4_or_mov_file>\n");
